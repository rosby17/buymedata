...
export async function sendVerification(email: string, purpose: "register" | "verify", payload: Record<string, string>) {
  ...
  const link = new URL("/verify-email", process.env.NEXT_PUBLIC_SITE_URL);
  ...
  try {
    await transport.sendMail({from:process.env.SMTP_FROM,to:email,subject:"Confirmez votre adresse · Buy Me Data",
      text:"Confirmez votre adresse pour accéder à Buy Me Data :\n\n"+link.toString()+"\n\nCe lien expire dans 30 minutes et ne fonctionne qu’une fois. Si vous n’avez pas fait cette demande, ignorez cet e-mail."});
  } catch {
