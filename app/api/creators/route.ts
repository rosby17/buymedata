...
export async function GET() {
  const result = await query(`SELECT p.id, p.full_name AS name, p.avatar_url, p.role, p.username, p.bio, p.category,
    ...
    WHERE p.role = 'creator' AND p.onboarding_completed = true GROUP BY p.id ORDER BY p.created_at DESC`);
