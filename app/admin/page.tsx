...
export default function AdminDashboard() {
  ...
  return <div className="min-h-screen bg-[#fbf9f4] text-[#1b1c19]"><nav className="border-b border-[#e4bdbc] bg-white"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5"><Link href="/" className="flex items-center gap-2"><TopUpLogo className="h-8 w-8"/><span className="font-bold text-[#b20024]">Buy Me Data · Admin</span></Link><Link href="/app/dashboard" className="text-sm font-semibold text-[#5b403f]">Mon espace</Link></div></nav>
    <main className="mx-auto max-w-7xl px-5 py-10"><h1 className="text-3xl font-bold">Pilotage de la plateforme</h1><p className="mt-2 text-sm text-[#6f5a57]">Uniquement des données enregistrées dans PostgreSQL.</p>
      {error&&<div className="mt-8 rounded-2xl border border-[#e4bdbc] bg-white p-8 text-center"><p className="font-bold text-[#b20024]">{error}</p><Link href="/app/dashboard" className="mt-4 inline-block text-sm font-semibold underline">Retour au tableau de bord</Link></div>}
